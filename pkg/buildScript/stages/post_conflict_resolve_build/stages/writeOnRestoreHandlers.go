...
// Writes onRestore handlers as a single function to the target file.
func WriteOnRestoreHandlers(cfg *build_config.BuildManager) {
	var handlers []ulld_plugin.PluginEventHandler
	for i, p := range cfg.Plugins {
		a, ok := p.Events().OnRestoreMethod(i)
		if ok {
	...
	f := file.NewFileItem(cfg.Paths.OnRestoreMethodList())
